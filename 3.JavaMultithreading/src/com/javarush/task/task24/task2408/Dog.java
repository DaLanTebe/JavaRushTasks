...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            @Override
            public String say() {
//                DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("dd-MMM-yyyy EE");
//                return i < 1 ? getName() + " спит." + " " +  LocalDateTime.now().format(dateTimeFormatter):
//                        getName() + " лает г" + new String(new char[i]).replace("\0", "а") + "в!" + " " + LocalDateTime.now().format(dateTimeFormatter);
                if (i < 1)
                    return getName() + " спит.";
